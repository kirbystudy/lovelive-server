...
# 执行代码编译命令
RUN mvn -f /app/pom.xml clean package -Dmaven.test.skip=true -Dspring.profiles.active=prod
...
# 执行启动命令
CMD ["java", "-jar", "/app/lovelive-server-0.0.1.jar", "--spring.profiles.active=prod"]